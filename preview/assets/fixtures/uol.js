var institutionDirectives = [
	...
	},
	{
		...
		declaration: function() {
			return {
				template:
					'<list data="ctrl.list" configuration="ctrl.configuration" view="{{ctrl.view}}" service-name="{{ctrl.serviceName}}" modules="ctrl.modules" user="ctrl.user" name="{{ctrl.name}}" endpoint="{{ctrl.endpoint}}" loading="ctrl.loading" precomponents="ctrl.precomponents" component="ctrl.component" request-error="ctrl.requestError"></list>',
				scope: {
					id: '@',
					endpoint: '@',
					name: '@',
				...
				controller: function(
					...
					function validateDisCourse(_course) {
						return _course.classType.toLowerCase() === 'dis';
					};
					function getCourses(data) {
						...
						data.forEach(function(_course) {
							...
							_course.enableBookStoreButton = true;
							...
							_course.displayDateBar = validateDisCourse(_course);
						...
						if (ctrl.view == 'courses') {
							dashboardData.fetchUserCourses(User.viewAs.email).then(function(res){
								if (!res.errors) {
									...
									ctrl.list = { classes: courseConfigurationResults.data };
								}
								if (res.errors) {
